...
////// THIS HANDLER //////
//////////////////////////
export async function handleFileVideoShorthand(req, res) {
  try {
      ...
      if (!isMalicious) {
        ...
        if (fileData) {
          ...
          if (accessAllowed) {
            ...
            if (fileType === "video" || fileData.processing === 1) {
                ...
                for (const key in sizesData) {
                  if (sizesData.hasOwnProperty(key)) {
                    if (key !== "original") { // Assuming you still want to skip 'original'
                      ...
                      fallbackFile = sizesData[key].file_location;
                    }
                  }
                }
...
export default handleFileVideoShorthand;